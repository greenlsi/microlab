...
mod utils2;
use utils2::api_routes::{get_timer_info, pulsar_boton};
use utils2::init::*;
use utils2::websocket::start_websocket_server;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Inicializar Session (Conexión con QEMU)
    let (session_arc, rx_irq) = init_session(&qemu_url).await?;
    ...
    let routes = pulsar_boton(session_arc.clone(), periferico.clone())
        .or(get_timer_info(session_arc.clone(), periferico.clone()))
